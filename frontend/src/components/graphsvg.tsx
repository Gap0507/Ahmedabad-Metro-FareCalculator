...
const VIEWBOX_WIDTH = 743.74;
const VIEWBOX_HEIGHT = 445.16;
const ROUTE_CAMERA_SCALE = 6.3;
const MIN_MAP_SCALE = 0.9;
...
const makeShortsScriptSegments = (
  ...
  const scripts = {
    en: [
      ...
      { text: 'Plan your ahmedabad Metro trip at metro dot coolhead dot in.', stationId: route.to },
    ],
    hi: [
      { text: routeIntro, stationId: route.from },
      { text: directionSentence, stationId: firstDirection?.stationId || route.from },
...
function SvgComponent({
  ...
  useLayoutEffect(() => {
    ...
    svg.querySelectorAll<SVGTextElement>('text[id^="lbl_"]').forEach((label) => {
      const extractedStationId = label.id.replace('lbl_', '');
      const station = stationLabels.find((item) => item.id === extractedStationId);
  ...
  const handleMapClick = React.useCallback((event: React.MouseEvent<SVGSVGElement>) => {
    ...
    const closestStation = Object.entries(stationCoordinates).reduce<{
      ...
      if (distance > 7 || (closest && closest.distance <= distance)) return closest;
  ...
  const downloadFullMapSvg = React.useCallback(async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = 'ahmedabad-metro-full-map.svg';
      link.click();
  ...
  const downloadRouteVideo = React.useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `ahmedabad-metro-route-${animationMode}.webm`;
      link.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 0);
    } catch (error) {
      console.error(error);
  ...
  const drawShortsFrame = React.useCallback(async (
    ...
    context.fillText('ahmedabad METRO ROUTE', 96, 154);
  ...
  const downloadYoutubeShortsVideo = React.useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `ahmedabad-metro-shorts-${route.from}-to-${route.to}.mp4`.toLowerCase();
      link.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 0);
    } catch (error) {
      console.error(error);
          ...
          <>
            {selectedStationPoint ? (
              <g transform={`translate(${selectedStationPoint.x} ${selectedStationPoint.y})`} pointerEvents="none">
                <circle r={8} fill="#dc2626" opacity={0.16}>
                ...
                <circle r={3.5} fill="#dc2626" stroke="#fff" strokeWidth={1.2} />
              </g>
            ) : null}
            {path ? (
              <>
                <path ref={routePathRef} stroke="transparent" d={path} fill="none" />
                <path className="route-highlight-halo" stroke="white" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" fill="none" d={path} />
                <path className="route-highlight-line" stroke="#111827" strokeWidth={1.2} strokeLinecap="round" strokeLinejoin="round" fill="none" d={path} />
                <g
                  ref={trainRef}
                ...
                >
                  ...
                  <g className="train-headlight" pointerEvents="none" aria-hidden="true">
                    <path
                      d="M6.5 -2 C12 -4.5 18 -4 24 -1.5 L24 1.5 C18 4 12 4.5 6.5 2 Z"
